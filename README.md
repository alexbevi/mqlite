...
Brokers also checkpoint automatically after about 60 seconds once the broker reaches a brief quiet window with no command in flight. If the last client disconnects and the broker stays quiet, it now checkpoints promptly instead of waiting for idle shutdown, and a loaded broker will also checkpoint early when the WAL tail grows past its backlog threshold even if a client keeps the connection open. The broker snapshots those background checkpoints so later commands can keep running, and any writes that arrive after the snapshot stay in the WAL tail until the next checkpoint.
