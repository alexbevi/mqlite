...
- Equality, comparison, membership, negated-membership, `$all`, array-size, and modulus matching on dotted field paths via `$eq`, `$ne`, `$gt`, `$gte`, `$lt`, `$lte`, `$in`, `$nin`, `$all`, `$exists`, `$size`, and `$mod`.
