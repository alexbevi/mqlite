...
See [ARCHITECTURE.md](/Users/alex/Workspace/mqlite/ARCHITECTURE.md) for the storage, index, broker, and command-execution design details.
...
- Aggregation expression operators `$literal`, `$const`, `$expr`, `$cond`, scoped variable operators `$let`/`$map`/`$filter`/`$reduce`, literal field access and mutation via `$getField`/`$setField`/`$unsetField`, comparison operators `$cmp`/`$eq`/`$ne`/`$gt`/`$gte`/`$lt`/`$lte`, boolean composition via `$and`/`$or`/`$not`, boolean-array checks via `$allElementsTrue`/`$anyElementTrue`, membership via `$in`, arithmetic via `$add`/`$subtract`/`$multiply`/`$divide`/`$mod`, numeric transforms via `$abs`/`$ceil`/`$floor`/`$round`/`$trunc`, type introspection via `$type`/`$isNumber`, string concatenation via `$concat`, null coalescing via `$ifNull`, and array/document transforms via `$size`/`$isArray`/`$arrayElemAt`/`$first`/`$last`/`$concatArrays`/`$objectToArray`/`$arrayToObject`/`$mergeObjects` plus array-sequence helpers `$indexOfArray`/`$range`/`$reverseArray`/`$slice`.
