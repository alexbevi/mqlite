...
If a file has a bounded pending WAL tail, read commands now scan that tail against the published roots and only materialize the requested namespace when that collection was actually touched, instead of forcing full mutable-engine recovery before the first response.
