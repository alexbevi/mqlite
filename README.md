...
- Incremental CRUD apply batches touched record and index-entry updates in memory, merges only affected index entry vectors, keeps planner histogram stats incrementally, and avoids rebuilding query-only runtime index trees on every committed mutation.
...
- WAL frames and checkpoint snapshots now encode their control metadata in a compact CBOR envelope while embedded MongoDB documents remain exact raw BSON bytes, which trims overhead without changing page-level user-document storage.
- Persisted change events keep their document-bearing fields as raw BSON blobs in memory and in the local change-event log, materializing nested `Document`s only when `$changeStream` consumers read them.
...
- Recovery replays WAL on top of the newest valid checkpoint and can fall back to an older superblock when the latest checkpoint pages are damaged. Checkpoints carry forward unchanged collection, index, and change-event pages from the active snapshot, and dirty collections now diff their current record and index state against the last checkpoint so unchanged prefix/suffix record pages and byte-identical index trees can still be reused instead of being re-encoded wholesale. When the inactive checkpoint region is large enough, the next snapshot reuses that preserved snapshot/WAL space instead of appending another full snapshot to the end of the file.
