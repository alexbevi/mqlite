...
- Equality, comparison, membership, negated-membership, `$all`, `$not`, `$type`, regex matching, array-size, and modulus matching on dotted field paths via `$eq`, `$ne`, `$gt`, `$gte`, `$lt`, `$lte`, `$in`, `$nin`, `$all`, `$not`, `$exists`, `$type`, `$regex`/`$options`, `$size`, and `$mod`.
