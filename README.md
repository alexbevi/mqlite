...
Brokers also checkpoint automatically after about 60 seconds once the broker reaches a brief quiet window with no command in flight and no open client connection. If the last client disconnects, it checkpoints immediately instead of waiting for idle shutdown; WAL backlog pressure requests that publication, but the broker defers the file-destructive checkpoint until connections drain so active import streams do not race checkpoint truncation. Checkpoints publish a new page-root snapshot that rewrites only dirty collection/index page subtrees plus fresh namespace metadata after the first snapshot exists, so clean collection pages can be reused and later broker starts have much less WAL to replay. Any writes that arrive after a background published snapshot stay in the WAL tail until the next publish or foreground checkpoint.
