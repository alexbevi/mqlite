...
If a file has a bounded pending WAL tail, read commands now scan that tail against the published roots and only overlay the requested namespace when that collection was actually touched, instead of forcing full mutable-engine recovery before the first response. Insert-only WAL tails stay as a small delta overlay, so indexed point reads do not hydrate the checkpointed collection or secondary index.
Brokers also checkpoint automatically after about 60 seconds once the broker reaches a brief quiet window with no command in flight and no open client connection. If the last client disconnects, it checkpoints immediately instead of waiting for idle shutdown; WAL backlog pressure requests that publication, but the broker defers the automatic file-destructive checkpoint until connections drain so active import streams do not race checkpoint truncation. Explicit `mqliteCheckpoint` commands can request `"background": true` to queue publication while the client connection stays open; reads and writes continue while the publisher writes the captured snapshot, and writes that arrive after the capture remain in the WAL tail until the next publish or foreground checkpoint. Checkpoints publish a new page-root snapshot that rewrites only dirty collection/index page subtrees plus fresh namespace metadata after the first snapshot exists, so clean collection pages can be reused and later broker starts have much less WAL to replay.
...
| `mqlite bench trades-import --file <path> --fixture <path>` | Stream the trades NDJSON or `.zst` benchmark fixture through one broker connection and report structured import timings; add `--create-indexes` to build `ticker_1` and `ticket_1` in the same run. |
| `mqlite bench trades-read --file <path>` | Reuse one broker connection to measure warm trades `ticket`/`ticker` point-read and count latency percentiles. |
...
`mqlite command --debug` keeps the normal command reply on stdout and emits a structured debug report on stderr. That report combines client-side IPC and wire timings with broker-side server, query, storage, catalog, exec, and BSON spans, plus counters such as page-cache hits and misses, page bytes read, decompressed zstd blobs and bytes, WAL replay counts, per-mutation replay bytes and document counts, and lightweight process snapshots.
