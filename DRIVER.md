...
- If the file has a bounded pending WAL tail, the broker now scans that tail against the published roots and only materializes the requested namespace when that collection changed, instead of forcing full mutable-engine recovery before serving the read.
