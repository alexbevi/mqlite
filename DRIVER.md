...
- Once the broker is running, it checkpoints automatically after about every 60 seconds when dirty, but only after a brief quiet window with no command in flight. If the last client disconnects, it checkpoints immediately instead of waiting for idle shutdown. A loaded broker also requests a background publication as soon as a durable write pushes the WAL tail past its backlog threshold, even if a client keeps the IPC connection open, so later restarts stay on the clean fast path. Checkpoints publish a new page-root snapshot that reuses clean collection pages instead of rewriting the full durable page graph after the first snapshot exists. Background checkpoints are handed off to a worker so later commands can keep running, and writes that arrive after that published snapshot remain in the WAL tail until the next publish or foreground checkpoint.
