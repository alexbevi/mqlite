...
`mqlite command --debug` is the direct diagnostics path when a local file is slow: it keeps the command reply on stdout and emits a structured JSON report on stderr with client-side IPC and wire timings plus broker-side server, query, storage, catalog, exec, and BSON spans and counters.
