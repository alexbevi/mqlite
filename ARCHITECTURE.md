...
- Applying CRUD deltas batches touched record state and index-entry maintenance in memory, merges only the affected index entry vectors, keeps per-index planner histogram stats incrementally, and serves runtime index scans from the sorted entry vectors so writes do not have to rebuild a query-only in-memory tree on every mutation.
