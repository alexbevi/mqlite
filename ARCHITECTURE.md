...
- Storage also keeps an in-memory per-collection unique-key validation cache derived from the catalog at open time, then applies small insert/update/delete deltas to that cache after each durable write instead of rebuilding duplicate-key state from every unique index on every command.
